...
import { MsrNavigationItem, MsrVerticalNavigationAppearance, MsrVerticalNavigationMode, MsrVerticalNavigationPosition } from '@msr/components/navigation/navigation.types';
...
@Component({
    selector       : 'msr-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : msrAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'msrVerticalNavigation'
})
export class MsrVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: MsrVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: MsrVerticalNavigationMode = 'side';
    @Input() name: string = this._msrUtilsService.randomId();
    @Input() navigation: MsrNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: MsrVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output() readonly appearanceChanged: EventEmitter<MsrVerticalNavigationAppearance> = new EventEmitter<MsrVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<MsrVerticalNavigationMode> = new EventEmitter<MsrVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<MsrVerticalNavigationPosition> = new EventEmitter<MsrVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<MsrNavigationItem> = new ReplaySubject<MsrNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<MsrNavigationItem> = new ReplaySubject<MsrNavigationItem>(1);
    ...
    private _msrScrollbarDirectives!: QueryList<MsrScrollbarDirective>;
    private _msrScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any
    {
        /* eslint-disable @typescript-eslint/naming-convention */
        return {
            'msr-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`msr-vertical-navigation-appearance-${this.appearance}`]: true,
            'msr-vertical-navigation-hover'                          : this._hovered,
            'msr-vertical-navigation-inner'                          : this.inner,
            'msr-vertical-navigation-mode-over'                      : this.mode === 'over',
            'msr-vertical-navigation-mode-side'                      : this.mode === 'side',
            'msr-vertical-navigation-opened'                         : this.opened,
            'msr-vertical-navigation-position-left'                  : this.position === 'left',
            'msr-vertical-navigation-position-right'                 : this.position === 'right'
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for msrScrollbarDirectives
     */
    @ViewChildren(MsrScrollbarDirective)
    set msrScrollbarDirectives(msrScrollbarDirectives: QueryList<MsrScrollbarDirective>)
    {
        // Store the directives
        this._msrScrollbarDirectives = msrScrollbarDirectives;

        // Return if there are no directives
        if ( msrScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._msrScrollbarDirectivesSubscription )
        {
            this._msrScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._msrScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    msrScrollbarDirectives.forEach((msrScrollbarDirective) => {
                        msrScrollbarDirective.update();
                    });
                });
    }
    ...
    ngAfterViewInit(): void
    {
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.msr-vertical-navigation-item-active');
            ...
            else
            {
                ...
                this._msrScrollbarDirectives.forEach((msrScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !msrScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    msrScrollbarDirective.scrollToElement('.msr-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
     ...
     * @param item
     */
    openAside(item: MsrNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: MsrNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('msr-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('msr-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('msr-vertical-navigation-aside-overlay');
