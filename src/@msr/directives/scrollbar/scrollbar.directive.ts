...
import { ScrollbarGeometry, ScrollbarPosition } from '@msr/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[msrScrollbar]',
    exportAs: 'msrScrollbar'
})
export class MsrScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_msrScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() msrScrollbar: boolean = true;
    @Input() msrScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'msrScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.msrScrollbar = coerceBooleanProperty(changes.msrScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.msrScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'msrScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.msrScrollbarOptions.currentValue);
